...
slider0 = ADC(Pin(27, mode=Pin.IN))
slider1 = ADC(Pin(26, mode=Pin.IN))
...
for x in range(4):
    for y in range(4):
        keypad[x,y].color = (80, 80, 80)
time.sleep_ms(100)
for x in range(4):
    for y in range(4):
        keypad[x,y].color = (0, 0, 0)
time.sleep_ms(100)
...
for x in range(4):
    for y in range(4):
        keypad[x,y].color = (200, 0 ,0)
time.sleep_ms(100)

for x in range(4):
    for y in range(4):
        keypad[x,y].color = (0, 200 ,0)
time.sleep_ms(100)

for x in range(4):
    for y in range(4):
        keypad[x,y].color = (0, 0 ,200)
time.sleep_ms(100)

for x in range(4):
    for y in range(4):
        keypad[x,y].color = (100, 100, 0)
time.sleep_ms(100)

def i_in_recv(i, recv):
    if (i < 0) or (i >= 16):
        return False
    targ = ""
    if i < 9:
...
while True:
    ...
    val0 = slider0.read_u16() * (3.3 / 65535)
    val1 = slider1.read_u16() * (3.3 / 65535)
    for x in range(4):
        for y in range(4):
            ind = x + 4*y
            key = keypad[x,y]
            ...
            if key.is_pressed():
                keys_down.append(4*y + x)
                
            if key.is_pressed() or (i_in_recv(ind, recv)):
                if y == 0:
                    key.color = (200, 0, 0)
                elif y == 1:
                    key.color = (0, 200, 0)
    ...
    send = str(val0) + ":" + str(val1) + ":" + str(keys_down)
    if send != prev_send:
        # print(send)
        uart.write(send)
        prev_send = send
